...
**상황:** editable-lifecycle kernel 이 patch 타입을 자체 정의하면 zod-crud 와 어휘 분기.
...
**조치:** editable-lifecycle 도 v4 로 올림 (이번 iter). 합리적 선택 — zod v4 가 stable.
...
**평가:** 가족 패키지 (aria-kernel + zod-crud + editable-lifecycle) 를 함께 쓰는 소비자가
...
## F9 · editable-lifecycle 자체 — IME compositionend → insertText ops 단발 발행 (자체 해결)
...
## F10 · editable-lifecycle 자체 — 키스트로크 burst → 1 transaction (다음 iter 자체 해결)
...
## F11 · editable-lifecycle 자체 — Cmd+Z / Cmd+Shift+Z 키보드 → onUndo/onRedo 콜백 (자체 해결)
...
## F12 · editable-lifecycle 자체 — DOM Selection ↔ DocPos resolver (자체 해결)
...
## F13 · editable-lifecycle 자체 — 정방향 삭제 (Delete key, `deleteContentForward`) (자체 해결)
...
## F14 · editable-lifecycle 자체 — Range selection delete/replace (자체 해결, 단일 text block scope)
...
## F15 · editable-lifecycle 자체 — commitAtomic 후 DOM caret 복원 (자체 해결)
...
## F16 · editable-lifecycle 자체 — Cross-block range delete (atomic 포함, 자체 해결)
...
## F17 · editable-lifecycle 자체 — Shift+Enter 줄바꿈 (`insertLineBreak`/`insertParagraph`) (자체 해결)
...
## F18 · editable-lifecycle 자체 — root blur → trigger 자동 cancel (자체 해결)
...
## F19 · editable-lifecycle 자체 — atomic 양옆 caret 휴리스틱 정밀화 (자체 해결)
...
## F20 · editable-lifecycle 자체 — onKeyDown 의 `defaultPrevented` 가드 (자체 해결)
...
## F22 · editable-lifecycle 자체 — `minQueryLength` 옵션 (트리거 즉시 popover 지연) (자체 해결)
...
## F23 · editable-lifecycle 자체 — `aria-multiline` 기본값 정합 수정 (자체 해결)
...
- React 19 + Vite 6 + 가족 3 패키지 (editable-lifecycle, @p/aria-kernel, zod-crud)
